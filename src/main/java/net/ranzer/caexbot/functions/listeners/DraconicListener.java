...
public class DraconicListener extends ListenerAdapter{
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		...
		if(!event.getMessage().getContentRaw().toLowerCase().startsWith(BotCommand.getPrefix(event.getGuild())))
			return;
		
		if (event.getAuthor() != CaexBot.getJDA().getSelfUser()) {
			String[] args = event.getMessage().getContentRaw().split(" ");
			String command = args[0].replace(BotCommand.getPrefix(event.getGuild()), "").toLowerCase();
