...
import net.ranzer.caexbot.functions.raffle.commands.manage.BanUserCommand;
import net.ranzer.caexbot.functions.raffle.commands.manage.ChannelEnableCommand;
import net.ranzer.caexbot.functions.raffle.commands.manage.ModifyRolesCommand;
import net.ranzer.caexbot.functions.raffle.commands.run.*;
    ...
    public void process(String[] args, MessageReceivedEvent event) {
       ...
       if (args.length == 0) {
            if(raffles.containsKey(event.getTextChannel())) {
                event.getChannel().sendMessageEmbeds(statusEmbed(raffles.get(event.getTextChannel()))).queue();
            }
            return;
        }
